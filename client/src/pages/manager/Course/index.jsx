...
const ManagerCoursePage = () => {
  const {
    majorState: { majors },
    courseState: { courses_pagination, courseMajor, courseFind },
    dispatchCourse,
    dispatchAlert,
  } = useStore();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (filterCourse.id) {
        try {
          const response = await Course.getCoursesMajor(filterCourse.id);
        ...
        try {
          ...
          setIsLoad(false);
          dispatchCourse(
            actionsCourse.courseActions.get_courses_pagination(courses)
          );
        } catch (error) {
          console.log(error);
        }
    ...
    <div className="page bg-white">
      <div className="mx-10">
              ...
              {majors.map((major) => (
                <div
                  className={`w-full my-3 font-bold ${
                    major._id === filterCourse.id ? "active" : null
                  }`}
        ...
        <div className="mt-14">
          ...
          <div>
            <CourseDetail isOpen={isOpenInfo} onClose={handleClose} />

            <CreateCourse isOpen={isOpenCreate} onClose={handleClose} />

            <UpdateCourse isOpen={isOpenUpdate} onClose={handleClose} />
